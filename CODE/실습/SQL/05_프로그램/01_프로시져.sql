...
-- 파라미터 전달하여 프로시저 호출
CALL get_employee_salary_history(10002);
...
-- 해당 사원의 최근 발령일자와 급여를 빈환하는 프로시저를 정의하시오.
-- OUT : 발령일자, 급여

DROP PROCEDURE IF EXISTS get_last_date_salary;

DELIMITER //

CREATE PROCEDURE get_last_date_salary(
    IN p_emp_no INT,
...
-- 호출
CALL get_last_date_salary(20100, @last_date, @last_salary);
