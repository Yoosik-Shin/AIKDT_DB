...
SELECT CAST('123.321' AS DECIMAL) AS '정수부분만 추출';
...
-- 연도만 추출
SELECT CAST('2025-04-30' AS YEAR) AS '연도만 추출'
...
-- FORMAT(숫자, 소수점자리) / 반올림 해주나봄
-- 123456.789를 소수점 아래 둘째 자리까지 천단위 구분기호(,) 포함 출력
SELECT FORMAT(123456.789, 2) AS '포맷 숫자';
-- 123456.789를 정수만 천단위 구분기호(,) 포함 출력
SELECT FORMAT(123456.789, 0) AS '포맷 숫자';


-- DATE_FORMAT(날짜, 형식)
-- 오늘 날짜를 2025년 04월 30일 12시 00분 00초 형식으로 출력하시오.
SELECT DATE_FORMAT(now(), '%Y년%m월%d일 %H시%i분%s초') '현재날짜시간'
      ,DATE_FORMAT(now(), '%Y/%m/%d %H:%i:%s') '현재날짜시간'
      ,DATE_FORMAT(now(), '%Y-%m-%d %H:%i:%s') '현재날짜시간'
      ,DATE_FORMAT(now(), '%Y.%m.%d %H:%i:%s') '현재날짜시간'
