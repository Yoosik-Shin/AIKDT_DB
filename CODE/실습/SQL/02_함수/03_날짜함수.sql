...
SELECT *
FROM employees
WHERE TIMESTAMPDIFF(YEAR, hire_date, CURDATE()) >= 40;


-- 종강일로 부터 열흘 남은 날을 구해보시오.
-- 날짜 형식 기호
-- %Y : 연
-- %m : 월
-- %d : 일
...
SELECT DATE_SUB(
      STR_TO_DATE('2025-09-17', '%Y-%m-%d')
      , INTERVAL 10 DAY) AS '종강 10일 전'
      ,DATE_SUB(
      STR_TO_DATE('2025-09-17', '%Y-%m-%d')
      , INTERVAL 2 MONTH) AS '종강 2달 전'
      ,DATE_SUB(
      STR_TO_DATE('2025-09-17', '%Y-%m-%d')
      , INTERVAL 1 YEAR) AS '종강 1년 전'
FROM dual;
